...
export async function listExpenses(
  ...
  const sortBy = options?.sortBy && SORT_COLUMNS.includes(options.sortBy as typeof SORT_COLUMNS[number]) ? `e.${options.sortBy}` : 'e.expenseDate';
  ...
  Object.keys(params).forEach(k => {
    if (k.startsWith('filter_')) countParams[k] = params[k];
  });

  const [rows, countRows] = await Promise.all([
    query<Expense[]>(
      `SELECT e.id, e.organizationId, e.categoryId, e.description, e.amount,
    ...
    query<Array<{ total: number }>>(
      `SELECT COUNT(*) AS total FROM Expenses e WHERE e.organizationId = @organizationId${whereExtra}`,
      { params: countParams }
    ),
  ]);
...
export async function deleteExpense(
  ...
  await query(
    `DELETE FROM Expenses WHERE id = @id AND organizationId = @organizationId`,
    { params: { id, organizationId } }
  );
  return true;
}
