...
describe('New Architecture Features – Comprehensive', () => {
  ...
  describe('Drivers – Dual Pay Model', () => {
    ...
    it('GET /api/drivers sorts by percentageRate desc', async () => {
      ...
      const res = await request(app)
        ...
        .set(auth());
      expect(res.status).toBe(200);
      const rates = res.body.data.map((d: any) => d.percentageRate);
    ...
    it('DELETE driver twice returns 404 on second attempt', async () => {
      ...
      await request(app).delete(`/api/drivers/${createRes.body.id}`).set(auth());
      const res = await request(app).delete(`/api/drivers/${createRes.body.id}`).set(auth());
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('Units – Comprehensive Fields', () => {
    ...
    it('DELETE unit twice returns 404 on second attempt', async () => {
      ...
      expect(createRes.status).toBe(201);
      await request(app).delete(`/api/units/${createRes.body.id}`).set(auth());
      const res = await request(app).delete(`/api/units/${createRes.body.id}`).set(auth());
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('Jobs – sourceType and Carrier Fields', () => {
    ...
    beforeAll(async () => {
      if (!ctx) return;
      const companyRes = await request(app).post('/api/companies').set(auth()).send({ name: 'Jobs Test Co' });
      companyId = companyRes.body.id;
      const jtRes = await request(app).post('/api/job-types').set(auth()).send({ companyId, title: 'Test Haul', rateOfJob: 500 });
      jobTypeId = jtRes.body.id;
      const dispRes = await request(app).post('/api/dispatchers').set(auth()).send({ name: 'Jobs Test Disp', commissionPercent: 10 });
      dispatcherId = dispRes.body.id;
      const unitRes = await request(app).post('/api/units').set(auth()).send({ name: 'Jobs Test Unit' });
      unitId = unitRes.body.id;
      const driverRes = await request(app).post('/api/drivers').set(auth()).send({ name: 'Jobs Test Driver', hourlyRate: 25 });
    ...
    it('POST creates job with all associations (driver, unit, dispatcher)', async () => {
      ...
      // startTime/endTime are now stored as full UTC ISO strings (Eastern
      // input "08:00" → UTC of that wall-clock time). We just assert they
      // are parseable and ordered, not the wire format.
      expect(typeof res.body.startTime).toBe('string');
      expect(typeof res.body.endTime).toBe('string');
      expect(new Date(res.body.endTime).getTime()).toBeGreaterThan(
        new Date(res.body.startTime).getTime(),
      );
    ...
    it('DELETE job twice returns 404 on second attempt', async () => {
      ...
      expect(createRes.status).toBe(201);
      await request(app).delete(`/api/jobs/${createRes.body.id}`).set(auth());
      const res = await request(app).delete(`/api/jobs/${createRes.body.id}`).set(auth());
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('Invoices – Direct Company Invoicing', () => {
    ...
    beforeAll(async () => {
      if (!ctx) return;
      const companyRes = await request(app).post('/api/companies').set(auth()).send({ name: 'Invoice Co' });
      companyId = companyRes.body.id;
      const company2Res = await request(app).post('/api/companies').set(auth()).send({ name: 'Invoice Co 2' });
      company2Id = company2Res.body.id;
      const dispRes = await request(app).post('/api/dispatchers').set(auth()).send({ name: 'Invoice Disp' });
      dispatcherId = dispRes.body.id;
    });
    ...
    it('POST without invoiceNumber auto-generates one (returns 201)', async () => {
      // invoiceNumber is auto-generated by invoiceService.generateNextInvoiceNumber.
      // The route no longer requires it in the request body.
      if (!ctx) return;
      const res = await request(app)
        .post('/api/invoices')
        .set(auth())
    ...
    it('DELETE invoice twice returns 404 on second attempt', async () => {
      ...
      expect(createRes.status).toBe(201);
      await request(app).delete(`/api/invoices/${createRes.body.id}`).set(auth());
      const res = await request(app).delete(`/api/invoices/${createRes.body.id}`).set(auth());
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('JobInvoice – Validation', () => {
    ...
    let dispatcherBId: string;

    beforeAll(async () => {
      if (!ctx) return;
      const companyRes = await request(app).post('/api/companies').set(auth()).send({ name: 'JI Validation Co' });
      companyId = companyRes.body.id;
      const jtRes = await request(app).post('/api/job-types').set(auth()).send({ companyId, title: 'JI Haul' });
      jobTypeId = jtRes.body.id;
      const dispARes = await request(app).post('/api/dispatchers').set(auth()).send({ name: 'Disp A' });
      dispatcherAId = dispARes.body.id;
      const dispBRes = await request(app).post('/api/dispatchers').set(auth()).send({ name: 'Disp B' });
      dispatcherBId = dispBRes.body.id;
    });

    // Helper to create a job + invoice + add
    const createJob = async (date: string, opts: Record<string, unknown> = {}) => {
      const res = await request(app).post('/api/jobs').set(auth()).send({ jobDate: date, jobTypeId, ...opts });
      expect(res.status).toBe(201);
      return res.body;
    };
    const createInvoice = async (num: string, date: string, opts: Record<string, unknown> = {}) => {
      const res = await request(app).post('/api/invoices').set(auth()).send({ invoiceNumber: num, invoiceDate: date, ...opts });
      expect(res.status).toBe(201);
      return res.body;
    };
    ...
    it('rejects job already on another invoice', async () => {
      ...
      await request(app).post(`/api/invoices/${inv1.id}/jobs`).set(auth()).send({ jobId: job.id, amount: 500 });
    ...
    it('rejects duplicate job on same invoice', async () => {
      ...
      await request(app).post(`/api/invoices/${inv.id}/jobs`).set(auth()).send({ jobId: job.id, amount: 500 });
    ...
    it('accepts add without amount (inherits from job effectiveAmount)', async () => {
      // Updated in dynamic-calc sprint: `amount` is now OPTIONAL on
      // POST /invoices/:id/jobs. When omitted, the JobInvoice.amount
      // column is NULL and the line inherits the job's effectiveAmount
      // via the vJobInvoiceEffective view.
      if (!ctx) return;
    ...
    it('job can be added to different invoice after removal', async () => {
      ...
      // Add then remove
      await request(app).post(`/api/invoices/${inv1.id}/jobs`).set(auth()).send({ jobId: job.id, amount: 500 });
      await request(app).delete(`/api/invoices/${inv1.id}/jobs/${job.id}`).set(auth());
    ...
    it('GET lists all jobs on an invoice', async () => {
      ...
      await request(app).post(`/api/invoices/${inv.id}/jobs`).set(auth()).send({ jobId: job1.id, amount: 300 });
      await request(app).post(`/api/invoices/${inv.id}/jobs`).set(auth()).send({ jobId: job2.id, amount: 400 });
      ...
      const amounts = res.body.map((l: any) => l.amount).sort();
