...
describe('5rivers.server API', () => {
  ...
  beforeAll(async () => {
    try {
      testContext = await createTestOrgAndUser();
    } catch (e) {
  ...
  afterAll(async () => {
    if (testContext?.orgId && testContext?.userId) {
      await deleteTestData(testContext.orgId, testContext.userId);
    }
    await closePool();
  }, 30000);
  ...
  describe('Dispatchers', () => {
    it('GET /api/dispatchers without token returns 401', async () => {
      const res = await request(app).get('/api/dispatchers');
      expect(res.status).toBe(401);
    });
  ...
  describe('JobTypes', () => {
    it('GET /api/job-types without token returns 401', async () => {
      const res = await request(app).get('/api/job-types');
      expect(res.status).toBe(401);
    });
  ...
  describe('Driver payments', () => {
    it('GET /api/driver-payments without token returns 401', async () => {
      const res = await request(app).get('/api/driver-payments');
      expect(res.status).toBe(401);
    });
