...
import type { ComputeAmountInputs } from '../../services/job.service';

export interface CalcFixture {
  name: string;
  input: ComputeAmountInputs;
  expected: number | null;
  /** If true, omit from SQL view tests — the view can't faithfully replicate. */
  skipInSql?: boolean;
}
...
const HOURLY_CASES: CalcFixture[] = [
...
const TONNAGE_CASES: CalcFixture[] = [
...
const CROSSCUT_CASES: CalcFixture[] = [
...
export const SQL_FIXTURES = CALC_FIXTURES.filter((f) => !f.skipInSql);
