...
const driverPay_service_1 = require("../../services/driverPay.service");
...
beforeAll(async () => {
    ...
    const dh = await (0, factories_1.createDriver)(ctx.orgId, {
        name: 'Hourly Hank',
        payType: 'HOURLY',
        hourlyRate: 30,
    });
    ...
    const dp25 = await (0, factories_1.createDriver)(ctx.orgId, {
        name: 'Quarter Quinn',
        payType: 'PERCENTAGE',
        percentageRate: 25,
    });
    driverPct25 = dp25.id;
    const dp50 = await (0, factories_1.createDriver)(ctx.orgId, {
        name: 'Half Hannah',
        payType: 'PERCENTAGE',
        percentageRate: 50,
    });
    ...
    for (const [start, end] of [['08:00', '13:00'], ['08:00', '16:00'], ['08:00', '18:00']]) {
        await (0, factories_1.createJob)(ctx.orgId, jobTypeId, {
...
describe('listDriverPaySummaries — pay calculation accuracy', () => {
    let summaries;
    beforeAll(async () => {
        summaries = await (0, driverPay_service_1.listDriverPaySummaries)(ctx.orgId);
    });
    ...
    it('PERCENTAGE driver pay tracks JobType rate changes via vJobsEffective', async () => {
        ...
        await (0, connection_1.query)(`UPDATE JobTypes SET rateOfJob = 200 WHERE id = @id`, { params: { id: jobTypeId } });
        const after = await (0, driverPay_service_1.listDriverPaySummaries)(ctx.orgId);
    ...
    it('HOURLY driver pay does NOT change when JobType rate changes', async () => {
        const before = await (0, driverPay_service_1.listDriverPaySummaries)(ctx.orgId);
