...
beforeAll(async () => {
    ...
    const hourly = await (0, factories_1.createJobType)(ctx.orgId, companyId, {
        dispatchType: 'hourly',
        rateOfJob: 120,
    });
    hourlyJobTypeId = hourly.id;
    const load = await (0, factories_1.createJobType)(ctx.orgId, companyId, {
        dispatchType: 'load',
        rateOfJob: 200,
    });
...
describe('createJob — override-only amount semantics', () => {
    it('amount omitted → stored as NULL, effectiveAmount computed', async () => {
        ...
        const fetched = await (0, job_service_1.getJobById)(job.id, ctx.orgId);
...
describe('updateJob — three semantics for the amount field', () => {
    ...
    it('amount omitted → keeps existing override', async () => {
        const updated = await (0, job_service_1.updateJob)(ctx.orgId, {
            id: baseJobId,
...
describe('listJobs — joined fields and pagination', () => {
    it('projects effectiveAmount on every row', async () => {
        const result = await (0, job_service_1.listJobs)(ctx.orgId, (0, types_1.normalizePagination)({ limit: 100 }));
    ...
    it('respects organizationId filter (no cross-org leak)', async () => {
        ...
        const result = await (0, job_service_1.listJobs)(otherOrgFakeId, (0, types_1.normalizePagination)({ limit: 100 }));
...
describe('getJobById — projection completeness', () => {
    it('returns null for an id from another org', async () => {
        ...
        const result = await (0, job_service_1.getJobById)(job.id, '00000000-0000-0000-0000-000000000000');
