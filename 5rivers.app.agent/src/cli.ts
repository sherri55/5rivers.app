...
Commands:
  ...
  /quit | /exit        Exit`);
    ...
    if (modelMatch) {
      ...
      if (p === 'deepseek') process.env.DEEPSEEK_MODEL  = model;
      else if (p === 'gemini')   process.env.GEMINI_MODEL     = model;
      else if (p === 'groq')     process.env.GROQ_MODEL       = model;
      else if (p === 'ollama')   process.env.OLLAMA_MODEL      = model;
      else if (p === 'lmstudio') process.env.LMSTUDIO_TOOL_MODEL = model;
      clearProviderRegistry();
      const { resetConversation } = await import('./llm.js');
      resetConversation('cli', 'local');
