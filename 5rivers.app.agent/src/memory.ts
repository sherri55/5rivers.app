...
function save(mem: AgentMemory): void {
  try {
...
function patternKey(p: Pick<JobPattern, 'companyName' | 'dispatcherName' | 'jobTypeName'>): string {
 ...
 */
export function recordJobPattern(
  pattern: Omit<JobPattern, 'count' | 'lastSeen'>,
): void {
  const mem      = load();
  ...
  if (existing) {
    ...
    if (pattern.driverName) existing.driverName = pattern.driverName;
    if (pattern.unitName)   existing.unitName   = pattern.unitName;
  } else {
  ...
  if (mem.patterns.length > MAX_PATTERNS) mem.patterns.length = MAX_PATTERNS;
...
export function buildMemoryPromptSection(): string {
  ...
  const lines = top.map((p) => {
    const extras: string[] = [];
    if (p.driverName) extras.push(`driver: ${p.driverName}`);
    if (p.unitName)   extras.push(`unit: ${p.unitName}`);
 ...
 */
export function parseNameFromToolResult(result: string): string | undefined {
