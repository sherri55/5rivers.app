...
// ─── Image input shape (kept local to avoid circular import with llm.ts) ────
...
const stateKey = (p: string, u: string): string => `${p}:${u}`;

export function getPhase(platform: string, userId: string): Phase {
  return states.get(stateKey(platform, userId))?.phase ?? 'idle';
}

export function getState(platform: string, userId: string): PipelineState | undefined {
  return states.get(stateKey(platform, userId));
}

export function setState(platform: string, userId: string, state: Omit<PipelineState, 'updatedAt'>): void {
  states.set(stateKey(platform, userId), { ...state, updatedAt: Date.now() });
}

export function clearState(platform: string, userId: string): void {
...
export function getImages(platform: string, userId: string): PipelineImage[] {
...
export const VALIDATOR_TOOLS: ReadonlySet<string> = new Set([
  'list_companies',
  'list_dispatchers',
  'list_drivers',
  'list_units',
...
export const CREATOR_TOOLS: ReadonlySet<string> = new Set([
  // Create
  'create_company',
  'create_dispatcher',
  'create_driver',
...
export function getToolFilterForPhase(phase: Phase): ReadonlySet<string> | undefined {
  switch (phase) {
...
const TICKET_SCHEMA = {
  ...
  properties: {
    ...
    letterheadKind: {
      ...
      description:
        '"trucking" when the letterhead name contains Trucking / Haulage / Transport / Logistics / Carriers / Inc-style trucking words; ' +
    ...
    foreman: {
      type: ['string', 'null'],
      description:
        'Value of "Foreman" / "Supervisor" / "Site Contact" field — informational only, NEVER an entity.',
    },
    jobLocation: {
      type: ['string', 'null'],
      description:
        'Value of "Job #" / "Job/Loc" / "Loc" / "Site" / "Profile #" — a site code or location reference, NOT a customer name.',
    },
 ...
 */
export function buildHtmlParserMessage(html: string, userContext: string): Message {
...
export function buildValidatorPrompt(easternDate: string): string {
...
export function buildCreatorPrompt(easternDate: string): string {
 ...
 */
export function isCreateConfirmation(userMessage: string, history: Message[]): boolean {
  // (a) Explicit create directive — always a green light.
  if (CREATE_DIRECTIVE_RE.test(userMessage)) return true;

  // (b) Plain affirmation — needs context.
  if (!CONFIRM_RE.test(userMessage)) return false;
...
export async function routePipelineTurn(args: {
  ...
  if (images && images.length > 0) {
    ...
    addMessage(platform, userId, {
      role: 'user',
      content:
        `Below is the structured data already extracted from the uploaded document. Use this as your input — do NOT ask me to provide it again, it is right here:\n\n` +
        '```json\n' + parsed + '\n```\n\n' +
  ...
  if (currentPhase === 'validating') {
    ...
    if (isCreateConfirmation(userMessage, history)) {
      ...
      if (state) setState(platform, userId, {
        phase: 'creating',
      ...
      addMessage(platform, userId, {
        role: 'user',
        content:
          'Confirmed — please proceed. Use the resolved IDs from the validation summary above. ' +
