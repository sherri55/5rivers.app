...
export function createTelegramBot(botToken) {
    const bot = new Telegraf(botToken);
    bot.start((ctx) => {
        const registered = hasToken(PLATFORM, String(ctx.from.id));
        ctx.reply(`Welcome to 5Rivers Assistant! 🚛\n\n` +
            (registered
    ...
    bot.help((ctx) => {
        ctx.reply(`*5Rivers Assistant — Commands*\n\n` +
            ...
            `I'll parse your message, confirm details, and create the records for you.`, { parse_mode: 'Markdown' });
    });
    ...
    bot.on('text', async (ctx) => {
        ...
        if (!token) {
            ctx.reply('You need to register first. Use:\n/register <your-jwt-token>\n\nGet your token from the 5Rivers web app.');
            return;
        }
