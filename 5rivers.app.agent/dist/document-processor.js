...
async function processTicket(extraction, client, supervised) {
    ...
    if (dispatcher)
        createArgs.dispatcherId = dispatcher.id;
    if (driver)
        createArgs.driverId = driver.id;
    if (unit)
        createArgs.unitId = unit.id;
    if (extraction.startTime)
        createArgs.startTime = extraction.startTime;
    if (extraction.endTime)
        createArgs.endTime = extraction.endTime;
    if (extraction.startLocation)
        createArgs.startLocation = extraction.startLocation;
    if (extraction.endLocation)
        createArgs.endLocation = extraction.endLocation;
    if (extraction.ticketNumber)
        createArgs.ticketNumber = extraction.ticketNumber;
    if (supervised) {
        lines.push('');
