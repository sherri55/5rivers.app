...
function applyProfile(profile) {
    ...
    if (profile.model) {
        ...
        if (p === 'deepseek')
            process.env.DEEPSEEK_MODEL = profile.model;
        if (p === 'gemini')
            process.env.GEMINI_MODEL = profile.model;
        if (p === 'groq')
            process.env.GROQ_MODEL = profile.model;
        if (p === 'ollama')
            process.env.OLLAMA_MODEL = profile.model;
        if (p === 'lmstudio')
            process.env.LMSTUDIO_TOOL_MODEL = profile.model;
    }
    ...
    if (profile.host) {
        if (profile.provider === 'lmstudio')
            process.env.LMSTUDIO_HOST = profile.host;
        if (profile.provider === 'ollama')
            process.env.OLLAMA_HOST = profile.host;
    }
}
 ...
 */
export function applyConfig(argv) {
    ...
    if (args.model) {
        ...
        if (p === 'deepseek')
            process.env.DEEPSEEK_MODEL = args.model;
        if (p === 'gemini')
            process.env.GEMINI_MODEL = args.model;
        if (p === 'groq')
            process.env.GROQ_MODEL = args.model;
        if (p === 'ollama')
            process.env.OLLAMA_MODEL = args.model;
        if (p === 'lmstudio')
            process.env.LMSTUDIO_TOOL_MODEL = args.model;
    }
    if (args.reasoning !== undefined)
        process.env.DEEPSEEK_REASONING = String(args.reasoning);
    if (args.reasoningEffort)
        process.env.DEEPSEEK_REASONING_EFFORT = args.reasoningEffort;
 ...
 */
export function switchProfile(nameOrProvider) {
    const pf = loadProfiles();
