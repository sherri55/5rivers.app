...
interface DriverJobsViewModalProps {
...
export const DriverJobsViewModal = ({ trigger, driver }: DriverJobsViewModalProps) => {
  const { data, loading, error } = useQuery(GET_DRIVER, {
    variables: { id: driver.id },
    skip: !driver.id
  })

  const jobs = data?.driver?.jobs || []
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Jobs for {driver.name}</DialogTitle>
        </DialogHeader>
        ...
        {!loading && !error && (
          <div className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Showing {jobs.length} jobs for this driver
            </div>
            ...
            {jobs.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">This driver doesn't have any jobs yet.</p>
              </div>
            ) : (
              jobs.map((job: any) => (
                      ...
                      {job.calculatedHours && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <div>
                            ...
                            <div className="font-medium text-foreground">{job.calculatedHours} hrs</div>
                          </div>
                        </div>
                      )}
                    </div>
                      ...
                      {job.unit && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <div>
                            ...
                            <div className="font-medium text-foreground">{job.unit.name}</div>
                          </div>
                        </div>
                      )}
                    </div>
