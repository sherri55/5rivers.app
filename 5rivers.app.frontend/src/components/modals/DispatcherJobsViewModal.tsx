...
export const DispatcherJobsViewModal = ({ trigger, dispatcher }: DispatcherJobsViewModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Jobs managed by {dispatcher.name}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-muted-foreground">No jobs found for {dispatcher.name}</p>
            </div>
          ) : (
            <>
              <div className="text-sm text-muted-foreground">
                Showing {jobs.length} job{jobs.length !== 1 ? 's' : ''} managed by {dispatcher.name}
              </div>
                  ...
                  <CardContent className="p-4">
                    ...
                    {job.jobType?.company?.name && (
                      <div className="flex items-center gap-2 text-sm mb-3">
                        ...
                        <span className="text-muted-foreground">{job.jobType.company.name}</span>
                      </div>
                    )}
