...
import { Calendar, DollarSign, User, Loader2, Building } from "lucide-react"
import { GET_JOB_TYPE } from "@/lib/graphql/jobTypes"

interface JobTypeJobsViewModalProps {
...
export const JobTypeJobsViewModal = ({ trigger, jobType }: JobTypeJobsViewModalProps) => {
  const [open, setOpen] = useState(false)
  
  const { data, loading, error } = useQuery(GET_JOB_TYPE, {
    variables: { id: jobType.id },
    skip: !open,
  })

  const jobs = data?.jobType?.jobs || []
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Jobs for {jobType.title}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-muted-foreground">No jobs found for {jobType.title}</p>
            </div>
          ) : (
            <>
              <div className="text-sm text-muted-foreground">
                Showing {jobs.length} job{jobs.length !== 1 ? 's' : ''} for {jobType.title}
              </div>
              
              {data?.jobType?.company && (
                <div className="flex items-center gap-2 text-sm mb-4 p-3 bg-muted/50 rounded-lg">
                  <Building className="h-4 w-4 text-muted-foreground" />
                  <span className="text-muted-foreground">Company:</span>
                  <span className="font-medium">{data.jobType.company.name}</span>
                </div>
              )}
