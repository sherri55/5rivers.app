...
const GRAPHQL_ENDPOINT =
  process.env.FIVERIVERS_GRAPHQL_URL ?? "http://localhost:4001/graphql";
  ...
  tools: [
    {
      name: "list_jobs",
      description:
        "List jobs with optional filters. Use for queries like 'show me jobs for 12th feb 2025', 'jobs for driver X', 'pending jobs'. Returns job list with date, amount, driver, status.",
      inputSchema: {
        type: "object",
        properties: {
          date: {
            type: "string",
            description:
              "Single date (e.g. '2025-02-12' or '12th feb 2025') - returns jobs on that day.",
          },
          dateFrom: {
            type: "string",
    ...
    },
    {
      name: "search_jobs",
      description:
        "Full-text search jobs (e.g. by ticket id, company name). Use when user asks to 'search for X'.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Get dashboard summary: total jobs, amount, invoices, comparison with previous period. Use for 'how many jobs this month', 'summary', 'overview'.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "list_invoices",
      description:
        "List invoices with optional filters. Use for 'show invoices', 'invoices for February'.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  try {
    if (name === "list_jobs") {
      ...
      if (safeArgs.date) {
        ...
        if (safeArgs.dateFrom) dateFrom = parseDateArg(String(safeArgs.dateFrom));
        if (safeArgs.dateTo) dateTo = parseDateArg(String(safeArgs.dateTo));
      }
      ...
      const result = await backend.getJobs(
        {
          dateFrom,
          dateTo,
          invoiceStatus: safeArgs.invoiceStatus as string | undefined,
        },
        { page: 1, limit }
      );
      const text =
    ...
    if (name === "get_dashboard_stats") {
      const year = safeArgs.year as number | undefined;
      const month = safeArgs.month as number | undefined;
    ...
    if (name === "list_invoices") {
      const status = safeArgs.status as string | undefined;
