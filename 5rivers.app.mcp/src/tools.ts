...
function fmtPaid(raw: unknown): string {
  if (raw === true) return '✅';
  if (raw === false) return '❌';
  return '—';
}
...
function formatJobDetail(job: Record<string, unknown>): string {
  ...
  if (job.from || job.to) lines.push(`Route:      ${job.from ?? '—'} → ${job.to ?? '—'}`);
  ...
  if (job.ticketNumber) lines.push(`Ticket:     ${job.ticketNumber}`);
...
const ROUTE_CONNECTORS_RE = new RegExp(
  `\\b(${ROUTE_CONNECTORS.join('|')})\\b`,
  'gi',
);
...
interface NamedRecord { id: string; name: string; [key: string]: unknown }
 ...
 */
function fuzzyScore(search: string, candidate: string): number {
  ...
  if (nc.includes(ns) || ns.includes(nc)) return 2;
  ...
  if (longer > 0 && longer <= 30) {
    ...
    if (sim >= 0.85)      typoSim = 1.4;
    else if (sim >= 0.75) typoSim = 0.9;
    else if (sim >= 0.65) typoSim = 0.4;
  }
...
type FuzzyAction =
  | { action: 'USE';    record: NamedRecord; note: string }
  | { action: 'CHOOSE'; candidates: Array<NamedRecord & { _score: number }> }
  | { action: 'NONE';   nearest: NamedRecord[]; totalCount: number };
 ...
 */
function fuzzyResolve(records: NamedRecord[], search: string): FuzzyAction {
 ...
 */
function formatFuzzyResult(result: FuzzyAction, entityLabel: string, search: string): string {
 ...
 */
function projectJob(job: Record<string, unknown>): Record<string, unknown> {
  ...
  let breaksDisplay: string | null = null;
  let breakMinutes = 0;
  if (typeof job.breaks === 'string' && job.breaks.trim()) {
    try {
      const arr = JSON.parse(job.breaks) as Array<{ start?: string; end?: string; tag?: string }>;
      if (Array.isArray(arr) && arr.length) {
        ...
        if (parts.length) breaksDisplay = parts.join('; ');
      }
...
const list_jobs: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No jobs found for the given filters.';
    const projected = (result.data as Record<string, unknown>[]).map(projectJob);
...
const get_job: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    return job ? formatJobDetail(projectJob(job as Record<string, unknown>)) : 'Job not found.';
  },
};

const search_jobs: ToolDefinition = {
  name: 'search_jobs',
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No jobs matched the search.';
    const projected = (result.data as Record<string, unknown>[]).map(projectJob);
    return formatJobTable(projected, { total: result.total as number });
  },
};

const get_dashboard_stats: ToolDefinition = {
  name: 'get_dashboard_stats',
  description:
    'Get dashboard summary: total jobs, revenue, expenses, profit, invoices. Use for "dashboard", "summary", "overview", "how many jobs this month", "what\'s our revenue".',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
const list_invoices: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No invoices found.';
    return json({ total: result.total, page: result.page, invoices: result.data });
  },
};

const list_drivers: ToolDefinition = {
  name: 'list_drivers',
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No drivers found.';
    const search = String(args.search ?? '').trim() || undefined;
    if (!search) return json({ total: result.data.length, drivers: result.data });
    const resolution = fuzzyResolve(result.data as NamedRecord[], search);
...
const list_companies: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No companies found.';
    const search = String(args.search ?? '').trim() || undefined;
    if (!search) return json({ total: result.data.length, companies: result.data });
    const resolution = fuzzyResolve(result.data as NamedRecord[], search);
    return formatFuzzyResult(resolution, 'company', search);
  },
};

const list_dispatchers: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No dispatchers found.';
    const search = String(args.search ?? '').trim() || undefined;
    if (!search) return json({ total: result.data.length, dispatchers: result.data });
    const resolution = fuzzyResolve(result.data as NamedRecord[], search);
    return formatFuzzyResult(resolution, 'dispatcher', search);
  },
};

const list_units: ToolDefinition = {
  name: 'list_units',
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No units found.';
    const search = String(args.search ?? '').trim() || undefined;
    if (!search) return json({ total: result.data.length, units: result.data });
    const resolution = fuzzyResolve(result.data as NamedRecord[], search);
    return formatFuzzyResult(resolution, 'unit', search);
  },
};

const list_carriers: ToolDefinition = {
  name: 'list_carriers',
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No carriers found.';
    const search = String(args.search ?? '').trim() || undefined;
    if (!search) return json({ total: result.data.length, carriers: result.data });
    const resolution = fuzzyResolve(result.data as NamedRecord[], search);
    return formatFuzzyResult(resolution, 'carrier', search);
  },
};

const list_job_types: ToolDefinition = {
  name: 'list_job_types',
  ...
  handler: async (client, args) => {
    const filters: Record<string, string> = {};
    if (args.companyId) filters.companyId = String(args.companyId);
    ...
    if (result.data.length === 0) return 'No job types found.';

    // Annotate job types with null rateOfJob (rate-pending marker).
    const annotated = (result.data as Record<string, unknown>[]).map((jt) => {
...
const list_expenses: ToolDefinition = {
  name: 'list_expenses',
  ...
  handler: async (client, args) => {
    const filters: Record<string, string> = {};
    if (args.categoryId) filters.categoryId = String(args.categoryId);
    if (args.startDate) filters.startDate = parseDateArg(String(args.startDate));
    if (args.endDate) filters.endDate = parseDateArg(String(args.endDate));
    ...
    if (result.data.length === 0) return 'No expenses found.';
    return json({ total: result.total, page: result.page, expenses: result.data });
  },
};

const list_expense_categories: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (result.data.length === 0) return 'No expense categories found.';
    const search = String(args.search ?? '').trim() || undefined;
    if (!search) return json({ total: result.data.length, categories: result.data });
    const resolution = fuzzyResolve(result.data as NamedRecord[], search);
    return formatFuzzyResult(resolution, 'expense category', search);
  },
};

const get_monthly_profit: ToolDefinition = {
...
const get_expenses_by_category: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const data = await client.analytics.expensesByCategory(
      args.startDate ? parseDateArg(String(args.startDate)) : undefined,
      args.endDate ? parseDateArg(String(args.endDate)) : undefined,
    );
    return json(data);
  },
};
...
function normaliseBreaks(value: unknown): string | null {
  ...
  const cleaned: Array<{ start: string; end: string; tag: string }> = [];
  for (const b of arr) {
    ...
    if (start && end && start < end) cleaned.push({ start, end, tag: tag || 'Break' });
  }
  return cleaned.length ? JSON.stringify(cleaned) : null;
}

/** Convert "12:30", "12:30 PM", "12:30PM", "12 PM" → "HH:MM" 24-hour. */
function to24h(v: unknown): string {
...
const create_job: ToolDefinition = {
  name: 'create_job',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      breaks: {
        type: 'array',
        description:
          'Hourly-job unpaid breaks. Each entry: {"start":"HH:MM","end":"HH:MM","tag":"Lunch"}. ' +
  ...
  handler: async (client, args) => {
    ...
    for (const [k, v] of Object.entries(args)) {
      ...
      if (k === 'jobDate')         data[k] = parseDateArg(String(v));
      else if (k === 'ticketNumber') data['ticketIds'] = v;
      else if (k === 'breaks')       data['breaks'] = normaliseBreaks(v);
      else                           data[k] = v;
    }
...
const update_job: ToolDefinition = {
  name: 'update_job',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      breaks: {
        type: 'array',
        description:
          'Replace the job\'s unpaid breaks. Each entry: {"start":"HH:MM","end":"HH:MM","tag":"Lunch"}. ' +
...
const create_job_type: ToolDefinition = {
...
const update_job_type: ToolDefinition = {
...
const create_driver: ToolDefinition = {
  name: 'create_driver',
...
const create_dispatcher: ToolDefinition = {
...
const create_unit: ToolDefinition = {
  name: 'create_unit',
...
const create_carrier: ToolDefinition = {
  name: 'create_carrier',
...
const create_expense: ToolDefinition = {
  name: 'create_expense',
  ...
  handler: async (client, args) => {
    ...
    for (const [k, v] of Object.entries(args)) {
      if (v !== undefined && v !== null && v !== '') {
        if (k === 'expenseDate') data[k] = parseDateArg(String(v));
        else data[k] = v;
      }
    }
...
const create_expense_category: ToolDefinition = {
...
const create_invoice: ToolDefinition = {
  name: 'create_invoice',
  ...
  handler: async (client, args) => {
    ...
    for (const [k, v] of Object.entries(args)) {
      if (v !== undefined && v !== null && v !== '') {
        if (k === 'invoiceDate') data[k] = parseDateArg(String(v));
        else data[k] = v;
      }
    }
...
const update_invoice: ToolDefinition = {
  name: 'update_invoice',
  ...
  handler: async (client, args) => {
    ...
    for (const [k, v] of Object.entries(rest)) {
      if (v !== undefined && v !== null && v !== '') {
        if (k === 'invoiceDate' || k === 'dueDate' || k === 'paidAt') data[k] = parseDateArg(String(v));
        else data[k] = v;
      }
    }
...
const delete_invoice: ToolDefinition = {
  name: 'delete_invoice',
...
const update_driver: ToolDefinition = {
  name: 'update_driver',
...
const update_unit: ToolDefinition = {
  name: 'update_unit',
...
const update_carrier: ToolDefinition = {
  name: 'update_carrier',
...
const update_dispatcher: ToolDefinition = {
...
const delete_job: ToolDefinition = {
  name: 'delete_job',
...
const list_driver_payments: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (args.driverId) filters.driverId = String(args.driverId);
    if (args.status)   filters.status   = String(args.status);
    if (args.dateFrom) filters.dateFrom = String(args.dateFrom);
    if (args.dateTo)   filters.dateTo   = String(args.dateTo);
    const result = await client.driverPayments.list({
      page: args.page ? Number(args.page) : 1,
      limit: args.limit ? Number(args.limit) : 50,
      filters,
    });
    if (result.data.length === 0) return 'No driver payments found.';
    return json({ total: result.total, page: result.page, totalPages: result.totalPages, payments: result.data });
  },
};

const create_driver_payment: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    for (const [k, v] of Object.entries(args)) {
      if (v !== undefined && v !== null && v !== '') {
        if (k === 'paymentDate') data[k] = parseDateArg(String(v));
        else data[k] = v;
      }
    }
...
const get_job_driver_pay: ToolDefinition = {
...
const set_job_driver_pay: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const { jobId, ...rest } = args as Record<string, unknown>;
    const data: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(rest)) {
      if (v !== undefined && v !== null && v !== '') data[k] = v;
    }
...
const get_next_invoice_number: ToolDefinition = {
...
const get_invoice_jobs: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (!jobs || (Array.isArray(jobs) && jobs.length === 0)) return 'No jobs attached to this invoice.';
    return json(jobs);
  },
};

const add_job_to_invoice: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const data: Record<string, unknown> = { jobId: args.jobId };
...
const remove_job_from_invoice: ToolDefinition = {
...
const get_revenue_by_company: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const data = await client.analytics.revenueByCompany(
...
const get_revenue_by_driver: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const data = await client.analytics.revenueByDriver(
      args.startDate ? String(args.startDate) : undefined,
      args.endDate   ? String(args.endDate)   : undefined,
    );
    return json(data);
...
const get_payment_status_summary: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const data = await client.analytics.paymentStatus(
      args.startDate ? String(args.startDate) : undefined,
      args.endDate   ? String(args.endDate)   : undefined,
    );
    return json(data);
...
const get_top_job_types: ToolDefinition = {
  ...
  handler: async (client, args) => {
    const data = await client.analytics.topJobTypes(
      args.startDate ? String(args.startDate) : undefined,
      args.endDate   ? String(args.endDate)   : undefined,
...
const get_monthly_revenue: ToolDefinition = {
...
const get_update_expense: ToolDefinition = {
  name: 'update_expense',
...
const mark_job_paid: ToolDefinition = {
  name: 'mark_job_paid',
  ...
  handler: async (client, args) => {
    const id = String(args.id);
    const data: Record<string, unknown> = {};
    if (args.jobPaid    !== undefined) data.jobPaid    = args.jobPaid;
    if (args.driverPaid !== undefined) data.driverPaid = args.driverPaid;
    if (Object.keys(data).length === 0) data.jobPaid = true; // default: mark job paid
    const job = await client.jobs.update(id, data);
 ...
 */
const mark_job_paid_by_date: ToolDefinition = {
  name: 'mark_job_paid_by_date',
  description:
    'Find a job by work date + dispatcher, verify the paid amount is within tolerance, then mark it as received (jobPaid=true). ' +
  ...
  handler: async (client, args) => {
    ...
    let bestJob: (typeof result.data)[0] | null = null;
    let bestVariance = Infinity;

    for (const job of result.data) {
      if (job['amount'] == null) continue;
    ...
    if (bestVariance > tolerance) {
      return (
        `FLAGGED: Best match on ${date} is job ${jobId} ` +
    ...
    if (bestJob['jobPaid']) {
      return (
        `ALREADY PAID: Job ${jobId} on ${jobDate} (system: $${jobAmount}) ` +
    ...
    return (
      `✅ MARKED: Job ${jobId} | date: ${jobDate} | ` +
      `system: $${jobAmount} | paid: $${paidAmount} | variance: ${bestVariance.toFixed(1)}%`
    );
  },
};

const mark_invoice_received: ToolDefinition = {
  ...
  handler: async (client, args) => {
    ...
    if (args.paidAt)     data.paidAt     = parseDateArg(String(args.paidAt));
    if (args.paidAmount) data.paidAmount = args.paidAmount;
